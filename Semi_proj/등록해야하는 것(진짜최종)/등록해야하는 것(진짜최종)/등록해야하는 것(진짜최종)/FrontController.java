...
//		System.out.println("~~~ 확인용 => 서블릿 FrontController 의  init(ServletConfig config) 메소드가 실행됨.");
		...
		String props = config.getInitParameter("propertyConfig"); //value값인 Command.properties가 나옴
//		System.out.println("~~~ 확인용 props => " + props);
		
		
		try {
			FileInputStream fis = new FileInputStream(props); //특정파일에 있는 내용을 읽어오기 위한 용도로 쓰이는 객체
...
//					~~~ 확인용 value => test.controller.Test1Controller
				...
				if(className != null) {
					...
					// 생성자로부터 실제 객체(인스턴스)를 생성
					Object obj = constrt.newInstance();
					/*
						### 확인용 : Test2Controller 기본생성자 호출됨 ###
						확인용 : Test1Controller 기본생성자 호출됨
					*/
			...
			System.out.println("C:/Users/sist/git/Semi_project/Semi_proj/src/main/webapp/WEB-INF/Command.properties 파일이 존재하지 않습니다.");
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void requestProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action == null) {
			...
			try {
				action.execute(request, response); //자식클래스(해당)에서 interface의 메소드를 구현했기 때문
				
				boolean bool = action.isRedirect();
				String viewPage = action.getViewPage();
				
				if(!bool) { //isRedirect가 false => forward방식
				...
				else { //isRedirect가 true => sendRedirect방식
