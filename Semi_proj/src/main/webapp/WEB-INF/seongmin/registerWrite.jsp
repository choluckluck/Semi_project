...
<script>
	...
	let id_check = true;
	let pwd_check = true;
	let pwdcheck_check = true;
	let name_check= true;
	let post_check = true;
	let mobile_check = true;
...
<%-- 		// 1. 아이디 확인
		...
		$("input[name=name]").blur((e)=>{
			...
			if(name == ""){ //입력하지 않거나 공백만 입력한 경우
				$target.parent().find(".register_warning").hide();
				$target.parent().find(".register_approval").hide();
				$target.val("${sessionScope.loginuser.name}");
...
//				$target.parent().find(".register_warning").show();
		...
		$("input[name=mobile2]").blur((e)=>{
			...
			if(mobile_m == ""){ //입력하지 않거나 공백만 입력한 경우
				$target.val("${ fn:substring(sessionScope.loginuser.mobile, 3, 7)}");				
				mobile_check = true;
//				$target.parent().find(".register_warning").show();
			}
			else {
		...
		$("input[name=mobile3]").blur((e)=>{
			...
			if(mobile_l == ""){ //입력하지 않거나 공백만 입력한 경우
				$target.val("${ fn:substring(sessionScope.loginuser.mobile, 7, 11)}");				
				mobile_check = true;
//				$target.parent().find(".register_warning").show();
			}
			else {
		...
		$("input[name=email]").change((e)=>{
			...
			if(email == "") { //입력하지 않거나 공백만 입력한 경우
				$target.parent().find(".register_warning").hide();			
				$target.val("${sessionScope.loginuser.email}");
		...
		const birthday_yy = birthday.substring(0,4);		
		const birthday_mm = birthday.substring(5,7);
		const birthday_dd = birthday.substring(8);
		...
		for(var i=1; i<=12; i++) {
			   if(i<10) {
			      if(i == Number(birthday_mm)) {
			         mm_html += "<option selected>0"+i+"</option>";
			      }
			      else {
			      ...
			      if(i == Number(birthday_mm)) {
			         mm_html += "<option selected>"+i+"</option>";
			      }
			      else {
		...
		for(var i=1; i<=31; i++) {
		   if(i<10) {
		      if(i == Number(birthday_dd)) {
		         dd_html += "<option selected>0"+i+"</option>";
		      }
		      else {
		      ...
		      if(i == Number(birthday_dd)) {
		         dd_html += "<option selected>"+i+"</option>";
		      }
		      else {
...
/* 				// 마케팅 수신 여부 체크
				...
				const frm = document.memberEditForm;
//				frm.marketing_yn.value = val;

				// form 값 전송
				frm.action = "<%= ctxPath%>/seongmin/member/memberEdit.sue";
				frm.method = "post";
				frm.submit(); 
			}
		});
  ...
  <div id="container">
 	<div class="row">
	   ...
	   <div class="col-9">
		<br><br>
			<form name="memberEditForm">
				<table id="register_table" class="container register_table">
						<tr>
							<td colspan="4" id="register_text">회원정보 수정</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								<input type="text" id="userid" name="userid" class="register_input w-100" value="${sessionScope.loginuser.userid }" readonly/>
						...
						<tr>
							...
							<td>
								...
								-->
								<input id="pwdinput" type="password" name="pwd" class="register_input w-100"  />
								<div id="pwd_approval" class="register_warning">사용 가능한 비밀번호입니다.</div> 
								<div id="pwd_warning" class="register_warning">새로운 비밀번호를 입력하세요.</div>
						...
						<tr>
							...
							<td>
								<input type="text" name="name" class="register_input w-100" value="${sessionScope.loginuser.name}"/>
						...
						<tr>
							...
							<td>
								<input type="number" id="postcode" name="postcode" class="zipcode_input w-25" onClick="zipcodeSearch()" value="${sessionScope.loginuser.postcode}" readonly/>
								<button type="button" class="zipcode_btn w-25" onClick="zipcodeSearch()" >우편번호</button>
								<div class="mt-2"></div>
								<input type="text" id="address" name="address" class="register_input w-100" onClick="zipcodeSearch()" value="${sessionScope.loginuser.address}" readonly/>
								<div class="register_description">기본주소</div>
								<input type="text" id="detailAddress" name="detailaddress" class="register_input w-100 mt-2" value="${sessionScope.loginuser.detailaddress}"/>
						...
						<tr>
							...
							<td>
								...
								<input type="number" oninput='handleOnInput(this, 4)' name="mobile2" class="register_input_phone" value="${ fn:substring(sessionScope.loginuser.mobile, 3, 7)}"/>
								<span class="division">-</span>
								<input type="number" oninput='handleOnInput(this, 4)' name="mobile3" class="register_input_phone" value="${ fn:substring(sessionScope.loginuser.mobile, 7, 11)}"/>
						...
						<tr>
							...
							<td>
								<input type="text" id="email" name="email" class="register_input w-100" value="${sessionScope.loginuser.email }"/>
						...
						<tr>
							...
							<td>
								<input type="number" id="birthyyyy" name="birthyyyy" class="register_input w-25" min="1900" max="2200" oninput="handleOnInput(this, 4)" value="${ fn:substring(loginuser.birthday, 0, 4)}"/>
...
<%-- 					<tr>
 ...
 --%>
						...
						<tr>
							<td colspan="2" class="pt-5 pb-2"><button type="button" id="join_btn" class="black" style="width:100%;">회원정보 수정</button></td>
						</tr>
						<tr>
